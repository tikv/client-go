...
func (c gcStatesClient) SetGlobalGCBarrier(ctx context.Context, barrierID string, barrierTS uint64, ttl time.Duration) (*pdgc.GlobalGCBarrierInfo, error) {
	...
	res := pdgc.NewGlobalGCBarrierInfo(barrierID, barrierTS, pdgc.TTLNeverExpire, startTime)
...
func (c gcStatesClient) GetGCState(ctx context.Context, opts ...pdgc.GCStatesAPIOption) (pdgc.GCState, error) {
	...
	return c.buildGCStateLocked(startTime, options.ExcludeGCBarriers), nil
}
...
func (c gcStatesClient) GetAllKeyspacesGCStates(ctx context.Context, opts ...pdgc.GCStatesAPIOption) (pdgc.ClusterGCStates, error) {
	...
	gcStates := map[uint32]pdgc.GCState{
		constants.NullKeyspaceID: c.buildGCStateLocked(startTime, options.ExcludeGCBarriers),
	}
	if options.ExcludeGlobalGCBarriers {
		return pdgc.NewClusterGCStatesWithoutGlobalGCBarriers(gcStates), nil
	}

	globalGCBarriers := make([]*pdgc.GlobalGCBarrierInfo, 0, len(c.inner.globalGCBarriers))
	for barrierID, barrierTS := range c.inner.globalGCBarriers {
		globalGCBarriers = append(globalGCBarriers, pdgc.NewGlobalGCBarrierInfo(barrierID, barrierTS, pdgc.TTLNeverExpire, startTime))
	}
	return pdgc.NewClusterGCStatesWithGlobalGCBarriers(gcStates, globalGCBarriers), nil
}

func (c gcStatesClient) buildGCStateLocked(startTime time.Time, excludeGCBarriers bool) pdgc.GCState {
	if excludeGCBarriers {
		return pdgc.NewGCStateWithoutGCBarriers(c.keyspaceID, c.inner.txnSafePoint, c.inner.gcSafePoint)
	}
	...
	return pdgc.NewGCStateWithGCBarriers(c.keyspaceID, c.inner.txnSafePoint, c.inner.gcSafePoint, gcBarriers)
}
