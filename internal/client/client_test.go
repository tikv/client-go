...
func TestFastFailWhenNoAvailableConn(t *testing.T) {
	...
	conn, err := client.getConnArray(addr, true)
	assert.Nil(t, err)
	_, err = sendBatchRequest(context.Background(), addr, "", conn.batchConn, req, time.Second)
	require.NoError(t, err)
	...
	start := time.Now()
	timeout := time.Second
	_, err = sendBatchRequest(context.Background(), addr, "", conn.batchConn, req, timeout)
	require.Error(t, err)
