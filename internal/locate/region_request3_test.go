...
func (s *testRegionRequestToThreeStoresSuite) TestReplicaSelector() {
	...
	for i := 0; i < len(replicaSelector.replicas)-1; i++ {
		...
		s.True(ok)
		s.Equal(regionStore.workTiKVIdx, replicaSelector.leaderIdx)
	...
	s.Nil(err)
	s.NotNil(rpcCtx)
	_, ok = replicaSelector.state.(*tryNewProxy)
	s.True(ok)
	s.Equal(regionStore.workTiKVIdx, replicaSelector.leaderIdx)
	...
	s.Nil(err)
	s.NotNil(replicaSelector)
	_, ok = replicaSelector.state.(*accessByKnownProxy)
	s.True(ok)
	s.Equal(regionStore.workTiKVIdx, replicaSelector.leaderIdx)
	_, err = replicaSelector.next(s.bo)
	s.Nil(err)
	...
	_, ok = replicaSelector.state.(*invalidLeader)
	s.True(ok)
