...
func (s *testRegionRequestToThreeStoresSuite) TestReplicaSelectorV2() {
	regionLoc, err := s.cache.LocateRegionByID(s.bo, s.regionID)
	s.Nil(err)
	s.NotNil(regionLoc)
	region := s.cache.GetCachedRegionWithRLock(regionLoc.Region)
	...
	s.Nil(err)
	s.NotNil(rpcCtx)
	s.NotEqual(region.GetLeaderPeerID(), replicaSelector.target.peer.Id)
	...
	s.NotEqual(replicaSelector.replicas[2].peer.Id, replicaSelector.proxy.peer.Id)
	AssertRPCCtxEqual(s, rpcCtx, replicaSelector.targetReplica(), replicaSelector.proxyReplica())
	s.Equal(replicaSelector.targetReplica().attempts, 1)
	s.Equal(replicaSelector.proxyReplica().attempts, 1)
	s.NotEqual(replicaSelector.targetReplica().store.getLivenessState(), reachable)
	s.Equal(replicaSelector.proxyReplica().store.getLivenessState(), reachable)

	// When the current proxy node fails, it should try another one.
	lastProxy := replicaSelector.proxy
	replicaSelector.onSendFailure(s.bo, nil)
	s.Equal(replicaSelector.proxyReplica().store.getLivenessState(), unreachable)
	rpcCtx, err = replicaSelector.next(s.bo, req)
	s.NotNil(rpcCtx)
	s.Nil(err)
	s.Equal(replicaSelector.replicas[2].peer.Id, replicaSelector.target.peer.Id)
	s.NotEqual(lastProxy.peer.Id, replicaSelector.proxy.peer.Id)
	s.NotEqual(region.GetLeaderPeerID(), replicaSelector.proxy.peer.Id)
	s.Equal(replicaSelector.targetReplica().attempts, 2)
	s.Equal(replicaSelector.proxyReplica().attempts, 1)
	s.Equal(replicaSelector.proxyReplica().store.getLivenessState(), reachable)
	...
	// Switch to tryNewProxy if the current proxy is not available
	replicaSelector.onSendFailure(s.bo, nil)
	for _, r := range replicaSelector.replicas {
		s.Equal(r.store.getLivenessState(), unreachable)
	}
	rpcCtx, err = replicaSelector.next(s.bo, req)
	s.Nil(err)
	...
	for i := 0; i < regionStore.accessStoreNum(tiKVOnly)-1; i++ {
		...
		// Shouldn't access the leader if followers aren't exhausted.
		s.NotEqual(region.GetLeaderPeerID(), rpcCtx.Peer.Id)
		AssertRPCCtxEqual(s, rpcCtx, replicaSelector.target, nil)
	...
	rpcCtx, err = replicaSelector.next(s.bo, req)
	s.Nil(err)
	s.Equal(replicaSelector.targetReplica().peer.Id, region.GetLeaderPeerID())
	AssertRPCCtxEqual(s, rpcCtx, replicaSelector.targetReplica(), nil)
	// All replicas are exhausted.
	rpcCtx, err = replicaSelector.next(s.bo, req)
	s.Nil(rpcCtx)
	...
	store.labels = labels
	for i := 0; i < 5; i++ {
		replicaSelector, err = newReplicaSelectorV2(cache, regionLoc.Region, req, WithMatchLabels(labels))
		s.NotNil(replicaSelector)
		s.Nil(err)
		rpcCtx, err = replicaSelector.next(s.bo, req)
		s.Nil(err)
		s.Equal(replicaSelector.replicas[accessIdx].peer.Id, replicaSelector.targetReplica().peer.Id)
		AssertRPCCtxEqual(s, rpcCtx, replicaSelector.replicas[accessIdx], nil)
	}
