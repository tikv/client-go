...
func TestValidateReadTSFromDifferentSource(t *testing.T) {
	...
	ts, err := o.GetTimestamp(context.Background(), &oracle.Option{})
	assert.NoError(t, err)
	lowResolutionTS, err := o.GetLowResolutionTimestamp(context.Background(), &oracle.Option{})
	assert.NoError(t, err)
	...
	go func() {
		firstResCh <- o.ValidateReadTS(context.Background(), ts+1, false, &oracle.Option{})
	}()
	...
	nextLowResolutionTS, err := o.GetLowResolutionTimestamp(context.Background(), &oracle.Option{})
	assert.NoError(t, err)
	...
	go func() {
		secondResCh <- o.ValidateReadTS(context.Background(), nextTS, false, &oracle.Option{})
	}()
	select {
	case err = <-firstResCh:
		assert.FailNow(t, fmt.Sprintf("expected to be blocked, but got result: %v", err))
