...
func (s *testKVSuite) SetupTest() {
	...
	s.Require().Nil(err)
	store.setGetMinResolvedTSByStoresIDs(func(ctx context.Context, ids []string) (uint64, map[uint64]uint64, error) {
		return 0, nil, nil
	})
...
func (s *testKVSuite) TestMinSafeTsFromPD() {
	mockClient := newStoreSafeTsMockClient(s)
	s.store.SetTiKVClient(mockClient)
	s.store.setGetMinResolvedTSByStoresIDs(func(ctx context.Context, ids []string) (uint64, map[uint64]uint64, error) {
		return 90, nil, nil
	})
	s.Eventually(func() bool {
		ts := s.store.GetMinSafeTS(oracle.GlobalTxnScope)
...
func (s *testKVSuite) TestMinSafeTsFromPDByStores() {
	...
	s.store.setGetMinResolvedTSByStoresIDs(func(ctx context.Context, ids []string) (uint64, map[uint64]uint64, error) {
		...
		for _, id := range ids {
			k, _ := strconv.ParseUint(id, 10, 64)
			m[k] = uint64(100) + k
		}
