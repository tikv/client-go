...
func (lr *LockResolver) BatchResolveLocks(bo *retry.Backoffer, locks []*Lock, loc locate.RegionVerID) (bool, error) {
	...
	detail.RequestSource = &util.RequestSource{
		RequestSourceInternal: true,
		RequestSourceType:     util.InternalTxnGC,
	}
	for _, l := range expiredLocks {
		...
		status, err := lr.getTxnStatus(bo, l.TxnID, l.Primary, 0, math.MaxUint64, true, false, l, &detail)
		if err != nil {
			return false, err
		}
		...
		if status.primaryLock != nil && status.primaryLock.UseAsyncCommit {
			resolveData, err := lr.checkAllSecondaries(bo, l, &status, &detail)
			if err == nil {
...
func (lr *LockResolver) resolveLocks(bo *retry.Backoffer, callerStartTS uint64, locks []*Lock, detail *util.ResolveLockDetail, forRead bool, lite bool) (int64, []uint64 /* canIgnore */, []uint64 /* canAccess */, error) {
	...
	resolve = func(l *Lock, forceSyncCommit bool) (TxnStatus, error) {
		...
		if status.primaryLock != nil && status.primaryLock.UseAsyncCommit && !forceSyncCommit {
			...
			status, err = lr.resolveAsyncCommitLock(bo, l, status, forRead, detail)
			if _, ok := errors.Cause(err).(*nonAsyncCommitLock); ok {
...
func (lr *LockResolver) checkSecondaries(bo *retry.Backoffer, txnID uint64, curKeys [][]byte, curRegionID locate.RegionVerID, shared *asyncResolveData, detail *util.ResolveLockDetail) error {
	...
	if regionErr != nil {
		...
		for regionID, keys := range regions {
			...
			if err = lr.checkSecondaries(bo, txnID, keys, regionID, shared, detail); err != nil {
				return err
			}
		}
		return nil
...
func (lr *LockResolver) resolveAsyncCommitLock(bo *retry.Backoffer, l *Lock, status TxnStatus, asyncResolveAll bool, detail *util.ResolveLockDetail) (TxnStatus, error) {
...
func (lr *LockResolver) checkAllSecondaries(bo *retry.Backoffer, l *Lock, status *TxnStatus, detail *util.ResolveLockDetail) (*asyncResolveData, error) {
	...
	for regionID, keys := range regions {
		...
		go func() {
			errChan <- lr.checkSecondaries(checkBo, l.TxnID, curKeys, curRegionID, &shared, detail)
		}()
	}
