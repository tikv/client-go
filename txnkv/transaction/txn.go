...
func (txn *KVTxn) GetTimestampForCommit(bo *retry.Backoffer) (_ uint64, err error) {
	if intest.InTest {
		ctx := bo.GetCtx()
		bo.SetCtx(context.WithValue(ctx, CtxInGetTimestampForCommitKey, txn.StartTS()))
		defer bo.SetCtx(ctx)
	}
	firstAttemptTS, err := txn.store.GetTimestampWithRetry(bo)
	if err != nil {
	...
	for ts := firstAttemptTS; ts <= txn.commitWaitUntilTSO; {
		...
		attempts++
		ts, err = txn.store.GetTimestampWithRetry(bo)
		if err != nil {
			break
		}
		lastAttemptTS = ts
