...
func TestCommitterCleanupIgnoresKillSignal(t *testing.T) {
	for _, mode := range []struct {
		...
		for _, useHandler := range []bool{false, true} {
			for _, cancelStore := range []bool{false, true} {
				t.Run(fmt.Sprintf("%s/handler=%t/cancelStore=%t", mode.name, useHandler, cancelStore), func(t *testing.T) {
					txn := newTestTxn(t, 1)
					...
					if useHandler {
						killed = 0
						vars.KillSignalHandler = testKillSignalHandler(func() error { return fmt.Errorf("query interrupted") })
					}
					txn.SetVars(vars)
