...
func (s *KVSnapshot) getSnapCacheWithoutLock(key []byte, returnCommitTS bool) (entry kv.ValueEntry, ok bool) {
	if entry, ok = s.mu.cached[string(key)]; ok {
		if !returnCommitTS {
			...
			// Though returning the commit ts if returnCommitTS is false is also acceptable,
			// we still set `entry.CommitTS = 0` to keep the behavior consistent with no cache hit case.
			entry.CommitTS = 0
			return
		}
