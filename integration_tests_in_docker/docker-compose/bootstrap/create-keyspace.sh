#!/bin/sh
set -eu

PD_ENDPOINT=${PD_ENDPOINT:-http://pd:2379}
KEYSPACE_NAME=local_normal
GC_CONFIG=gc_management_type=keyspace_level
system_output=/tmp/system-keyspace.$$.out
local_output=/tmp/local-keyspace.$$.out
error_output=/tmp/keyspace.$$.err

cleanup() {
    rm -f "${system_output}" "${local_output}" "${error_output}"
}
trap cleanup 0

show_keyspace() {
    /pd-ctl -u "${PD_ENDPOINT}" keyspace show name "$1"
}

has_json_field() {
    output_file=$1
    field=$2
    expected_value=$3
    compact=$(tr -d '\r\n\t ' < "${output_file}")
    case "${compact}" in
        *"\"${field}\":\"${expected_value}\""*) return 0 ;;
        *) return 1 ;;
    esac
}

is_enabled_keyspace() {
    has_json_field "$2" name "$1" &&
        has_json_field "$2" state ENABLED
}

is_valid_local_keyspace() {
    has_json_field "${local_output}" name local_normal &&
        has_json_field "${local_output}" state ENABLED &&
        has_json_field "${local_output}" gc_management_type keyspace_level
}

attempt=1
while [ "${attempt}" -le 120 ]; do
    if /pd-ctl -u "${PD_ENDPOINT}" member >"${error_output}" 2>&1; then
        break
    fi
    attempt=$((attempt + 1))
    sleep 1
done
if [ "${attempt}" -gt 120 ]; then
    echo "PD did not become ready at ${PD_ENDPOINT}" >&2
    cat "${error_output}" >&2 || true
    exit 1
fi

attempt=1
while [ "${attempt}" -le 120 ]; do
    if show_keyspace SYSTEM >"${system_output}" 2>"${error_output}" &&
        is_enabled_keyspace SYSTEM "${system_output}"; then
        break
    fi
    attempt=$((attempt + 1))
    sleep 1
done
if [ "${attempt}" -gt 120 ]; then
    echo "SYSTEM keyspace was not visible and enabled" >&2
    cat "${error_output}" "${system_output}" >&2 || true
    exit 1
fi

show_keyspace "${KEYSPACE_NAME}" >"${local_output}" 2>"${error_output}" || true
if has_json_field "${local_output}" name "${KEYSPACE_NAME}"; then
    echo "keyspace ${KEYSPACE_NAME} already exists"
else
    /pd-ctl -u "${PD_ENDPOINT}" keyspace create "${KEYSPACE_NAME}" --config "${GC_CONFIG}"
fi

attempt=1
while [ "${attempt}" -le 120 ]; do
    if show_keyspace "${KEYSPACE_NAME}" >"${local_output}" 2>"${error_output}" &&
        is_valid_local_keyspace; then
        break
    fi
    attempt=$((attempt + 1))
    sleep 1
done
if [ "${attempt}" -gt 120 ]; then
    echo "local_normal keyspace validation failed" >&2
    cat "${error_output}" "${local_output}" >&2 || true
    exit 1
fi

if ! show_keyspace SYSTEM >"${system_output}" 2>"${error_output}" ||
    ! is_enabled_keyspace SYSTEM "${system_output}"; then
    echo "SYSTEM keyspace validation failed" >&2
    cat "${error_output}" "${system_output}" >&2 || true
    exit 1
fi
